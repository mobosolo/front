...
class _ClientOrderHistoryScreenState extends ConsumerState<ClientOrderHistoryScreen> {
  ...
  Future<void> _fetchClientOrders() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = "Erreur lors du chargement de l'historique des commandes: ${e.toString()}";
        });
      }
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(title: Text('Mon Historique de Commandes')),
    ...
    if (_errorMessage != null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Mon Historique de Commandes')),
        body: Center(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              onTap: () {
                context.push('/order-confirmation/${order.id}');
              },
            ),
          );
        },
